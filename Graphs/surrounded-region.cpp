...
//a region is captured by flipping all o's and x's in that surrounded region

//travers the o components around the edges of the grid and change it to *
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            if(i==0 or i==n-1 or j==0 or j==m-1){
                if(A[i][j]=='O'){
                     change(A,i,j);//convert the following into *
